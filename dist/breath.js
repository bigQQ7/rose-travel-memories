...
export function breathFeatures(wave,frequency,sampleRate){
 ...
 const start=Math.max(1,Math.ceil(80/binHz)),end=Math.min(frequency.length,Math.floor(6500/binHz));
...
export function isBreath(f,noiseFloor){
 const broad=f.flatness>.045&&f.entropy>.58;
 const wind=f.lowRatio>.5&&f.entropy>.38&&f.peakRatio<.3;
 return f.rms>Math.max(.016,noiseFloor*2.8)&&(broad||wind);
}
